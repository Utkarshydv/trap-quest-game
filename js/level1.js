class Level1 extends Phaser.Scene {
  ...
  preload() {
    this.load.image("tiles", "assets/tileset/tiles_packed.png");
    ...
    this.load.image("player_jump", "assets/player/player_jump.png");
  ...
  create() {
    ...
    const trapCols = [6, 7, 14, 15, 22, 23];
    trapCols.forEach(x => {
      for (let y = 6; y <= 7; y++) {
        const tile = layer.putTileAt(0, x, y);
        tile.setCollision(true);
    ...
    this.physics.add.overlap(this.player, this.portal, () => this.scene.start("Level2"));
